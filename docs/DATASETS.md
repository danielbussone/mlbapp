...
| Lahman Hall of Fame (CSV / databank zip) | One row per inducted **player** (deduped) | `lahman_player_id` (= Lahman `playerID` / BBRef id) | `pnpm etl:hall-of-fame`: default **Internet Archive** snapshot of the old Chadwick Baseball Databank zip (GitHub zip **404** in 2026); optional ``MLBAPP_HALL_OF_FAME_CSV_URL`` / ``MLBAPP_HALL_OF_FAME_CSV_FILE`` / ``MLBAPP_BASEBALL_DATABANK_ZIP_URL`` | ~300+ rows total | On demand | MLB-focused list; archive snapshot is **2022-era** — refresh CSV URL when you need newer inductees | Lahman-style schema (open data lineage) | `hall_of_fame_player` | Join `dim_player` via `player_external_identifier` (`id_system='baseball_reference'`, `id_value=lahman_player_id`) after Chadwick load |
...
- **Directional OAA (outfield):** Savant’s **Directional Outs Above Average** leaderboard CSV (`statcast_outfield_directional_oaa` in pybaseball — same export as the site’s Download CSV) loads into **`savant_fielding_oaa_cell`** via **`pnpm etl:fielding-oaa`** ([`fielding_oaa_cell.py`](../etl/mlbapp_etl/fielding_oaa_cell.py), [STATCAST_REQUIREMENTS.md](./STATCAST_REQUIREMENTS.md) §11). Six `cell_id` values per player (`of_dir_back_left`, …). Not the same as pitch-level Statcast.
- **Infield OAA (directional buckets):** Savant **Outs Above Average** with **`pos=infield`** (`statcast_outs_above_average(year, 'if', …)` in pybaseball) loads into the same table via **`pnpm etl:fielding-oaa-if`** ([STATCAST_REQUIREMENTS.md](./STATCAST_REQUIREMENTS.md) §11b). Six cells per player (`if_dir_*` + `if_split_*`).
...
- **Expanded JAWS (fWAR):** `mv_fg_batter_jaws_cohort` / `mv_fg_pitcher_jaws_cohort` (Flyway **V26**, **V27** peak = **sum** of top seven season WAR) hold JAWS stats for rate-qualified careers; **V28** adds `mv_fg_batter_jaws_primary_pos` / `mv_fg_pitcher_jaws_primary_role` (no career PA / games floor); **V29** fixes those matviews (batting: coalesce PA from season current, PH/UTIL/UT→DH; pitching: SP/RP from career GS/G only, no peak-WAR join). **Rank vs Hall of Famers** (midrank) at that position compares the subject to **Hall of Famers** there (same MV + `hall_of_fame_player`, Flyway **V25**, `pnpm etl:hall-of-fame`). Refresh after `pnpm etl:fg`. API: `GET /players/:id/jaws-expanded?role=batting|pitching`. Numbers are **FanGraphs WAR**, not Baseball-Reference rWAR.
