-- Expose career counting H/AB from the same agg pass as other career stats (V10 view),
-- so API callers need not re-aggregate fg_batting_season_mlb_consolidated by id_fg.
...
COMMENT ON VIEW fg_batting_career_mlb IS
  'MLB batting career: walks from sum(bb_pct*pa); OBP = (H+walks+HBP)/(AB+walks+HBP+SF+SH) per FanGraphs career display (SF denominator only). career_h / career_ab = Σ H / Σ AB on consolidated seasons (same source as career_avg).';
