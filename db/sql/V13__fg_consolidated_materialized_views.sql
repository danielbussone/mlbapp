-- Replace MLB consolidated *season* views with materialized snapshots so card queries
-- filter on pre-aggregated rows instead of expanding the full fg_*_season_current pipeline.
-- Career views remain plain views over the MVs. Refresh after FanGraphs ETL (see etl/mlbapp_etl/fg.py).
...
-- ---------------------------------------------------------------------------
-- Batting consolidated (same logic as prior VIEW; V9 body)
-- ---------------------------------------------------------------------------
...
COMMENT ON MATERIALIZED VIEW fg_batting_season_mlb_consolidated IS
  'MLB batting one row per (id_fg, season): TOT/split merge; walks_bb_pct_pa = sum(bb_pct*pa). Snapshot — REFRESH after FanGraphs ETL.';
...
COMMENT ON MATERIALIZED VIEW fg_pitching_season_mlb_consolidated IS
  'MLB pitching one row per (id_fg, season): TOT/split merge. Snapshot — REFRESH after FanGraphs ETL.';
...
-- ---------------------------------------------------------------------------
-- Career views (unchanged definitions; read from MVs)
-- ---------------------------------------------------------------------------
