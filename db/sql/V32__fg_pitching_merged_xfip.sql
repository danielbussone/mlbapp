-- IP-weighted xFIP on merged MLB pitching stats for league percentiles (`fg_season_pit_xfip`).
-- Same pattern as xERA: IP-outs numerator/denominator + typed MAX(xfip) fallback.
--
-- ``xfip`` must be the *last* output column: PostgreSQL ``CREATE OR REPLACE VIEW`` cannot insert
-- a column mid-list (42P16: would shift ``tbf`` and appear as renaming columns).
...
COMMENT ON VIEW fg_pitching_season_mlb_merged_stats IS
  'MLB pitching season merge: IP-weighted xERA and xFIP, TBF-based K%/BB%, FIP-WAR (sum war) and RA9-WAR (sum stats_jsonb RA9-WAR); player_id from row or player_external_identifier (fangraphs).';
