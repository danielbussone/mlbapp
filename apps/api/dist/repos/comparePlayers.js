...
function parseStatsJsonObject(raw) {
    if (raw == null)
        return null;
...
function pickFromStatsJsonb(role, row) {
...
function slimPlayersForLlm(players) {
 ...
 */
export function stripComparePayloadForLlm(payload) {
    if (payload.error != null)
        return { ...payload };
    const rowCounts = (blocks) => {
        if (!Array.isArray(blocks))
            return [];
        return blocks.map((b) => {
            if (!b || typeof b !== 'object')
                return { player_id: null, row_count: 0 };
            const box = b;
            const rows = box.rows;
            return {
    ...
    const stripRows = (blocks, keys, role) => {
        if (!Array.isArray(blocks))
            return blocks;
        return blocks.map((b) => {
            if (!b || typeof b !== 'object')
            ...
            return {
                ...
                rows: rows.map((r) => {
                    ...
                    if (statsPick)
                        slim.stats_jsonb_pick = statsPick;
                    return slim;
                }),
            };
        });
    };
    const metaBase = payload.meta != null && typeof payload.meta === 'object'
        ? { ...payload.meta }
        : {};
