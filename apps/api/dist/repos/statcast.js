 ...
 */
export async function statcastPitcherPitchMixExtended(pool, pitcher_mlbam, game_year) {
    const y = clampYear(game_year);
    const { rows } = await pool.query(`
    WITH p AS (
      SELECT
 ...
 */
export async function statcastPitcherPitchMixExtendedByBatterStand(pool, pitcher_mlbam, game_year) {
    const y = clampYear(game_year);
    const { rows } = await pool.query(`
    WITH p AS (
      SELECT
...
export async function statcastPitcherVeloHistogram(pool, pitcher_mlbam, game_year, mphLo = 60, mphHi = 105) {
    const y = clampYear(game_year);
    const { rows } = await pool.query(`
    WITH p AS (
      SELECT
...
export async function statcastLeagueAvgVeloByPitchType(pool, game_year) {
    const y = clampYear(game_year);
    const { rows } = await pool.query(`
    SELECT
      pitch_type,
...
export async function statcastPitcherThrowsHand(pool, pitcher_mlbam, game_year) {
    const y = clampYear(game_year);
    const { rows } = await pool.query(`
 ...
 */
export async function statcastLeagueMovementByYear(pool, game_year, p_throws_hand = null) {
    const y = clampYear(game_year);
 ...
 */
export async function statcastBatterBatPathSummary(pool, batter_mlbam, game_year) {
    const y = clampYear(game_year);
    const { rows } = await pool.query(`
    WITH league_rows AS (
      SELECT
...
export async function statcastPitcherMixByYearRange(pool, pitcher_mlbam, yearFrom, yearTo) {
...
export async function statcastBatterBatPathTimeseries(pool, batter_mlbam, yearFrom, yearTo) {
    const y0 = clampYear(yearFrom);
    const y1 = clampYear(yearTo);
    const lo = Math.min(y0, y1);
    const hi = Math.max(y0, y1);
    ...
    for (let y = lo; y <= hi; y += 1)
        years.push(y);
    const rows = await Promise.all(years.map(async (gy) => {
 ...
 */
export function statcastSummaryHasRenderableData(role, payload, want) {
    if (role === 'pitcher') {
        if (want.mix) {
        ...
        if (want.mixExtended) {
            ...
            if ((Array.isArray(mx) && mx.length > 0) || (Array.isArray(mxs) && mxs.length > 0))
                return true;
        }
        if (want.veloDist) {
    ...
    if (want.battedBall) {
        const bb = payload.batted_ball;
        const bbe = Number(bb?.bbe ?? 0);
        if (Number.isFinite(bbe) && bbe > 0)
            return true;
    }
    if (want.batPath) {
        const bp = payload.bat_path;
