      ...
      parameters: {
        ...
        properties: {
          ...
          sort_metric: {
            type: 'string',
            description:
              'Must be an allowed key for the dataset (not Stuff+/Location+/Pitching+). Examples: career_war, war, k_pct, tto_rate_sum, def_runs, career_def_runs, era, fip.',
          },
 ...
 */
export function slimLeaderboardPayloadForLlm(payload: unknown): unknown {
  if (!payload || typeof payload !== 'object') return payload;
  const rec = payload as Record<string, unknown>;
  if (rec.error != null) return payload;
  const lb = rec.leaderboard;
  if (!lb || typeof lb !== 'object') return payload;
  const board = lb as Record<string, unknown>;
  const rows = board.rows;
  if (!Array.isArray(rows)) return payload;
  if (rows.length <= LLM_LEADERBOARD_ROW_CAP) return payload;
  return {
    ...rec,
    leaderboard: {
      ...board,
