...
function clampSeasonYear(n: number): number {
 ...
 */
const optionalLooseSeasonYear = z.preprocess((v: unknown) => {
  if (v === null || v === undefined) return undefined;
  if (typeof v === 'string') {
    const t = v.trim();
...
export const resolvePlayerArgsSchema = z
  ...
  .refine(
    (v) =>
      (v.name_query != null && String(v.name_query).trim() !== '') ||
      (v.key_mlbam != null && Number.isFinite(v.key_mlbam)) ||
      (v.id_fangraphs != null && Number.isFinite(v.id_fangraphs)),
    { message: 'Provide name_query, key_mlbam, or id_fangraphs' }
  );
