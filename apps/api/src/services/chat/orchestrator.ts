...
import { clipForChatLog, logToolRun, type ChatStreamLogger } from '../../lib/chatStreamLog.js';
...
/**
 * Provider-neutral chat driver: seeds the transcript, runs server-driven tool injection, then loops
 * `provider.chatRound` + tool execution until the model stops calling tools. All SSE emission and tool
 * orchestration lives here; the provider only owns wire-format translation for a single round.
 */
export async function streamChatWithTools(
  ...
  if (!(await provider.reachable())) {
    log?.info(
      { step: 'provider_unreachable', provider: provider.name },
      'chat provider unreachable; stub response'
    );
    write('token', { text: `[${provider.name} unreachable] ` });
    write('token', {
      text: `Stub: connect ${provider.name} to use tools. You said: ` + JSON.stringify(prompt),
    });
    return;
  }
  ...
  try {
    ...
    for (let round = 0; round < MAX_TOOL_ROUNDS; round++) {
      ...
      let { assistantMessage, hadToolCalls } = await provider.chatRound(
        messages,
        toolsThisRound,
        write,
        log,
        { phase: 'tool_loop', round, suppressAssistantTokens: Boolean(trailingTool) }
      );
      ...
      if (trailingTool) {
        ...
        if (!hadToolCalls && (looksLikeGenericRefusal(text) || text.trim() === '')) {
          ...
          messages.pop();
          ({ assistantMessage, hadToolCalls } = await provider.chatRound(
            messages,
            undefined,
            write,
            log,
      ...
      const toolCalls = assistantMessage.tool_calls as ProviderToolCall[];
    ...
    if (last && last.role === 'tool') {
      const { assistantMessage } = await provider.chatRound(
        messages,
        undefined,
        write,
        log,
