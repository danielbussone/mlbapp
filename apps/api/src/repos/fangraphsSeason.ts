...
export type FgRole = 'batting' | 'pitching';

export type FgSeasonLineInput = {
  player_id: number;
  role: FgRole;
  season?: number | null;
  ...
  limit?: number | null;
  /**
   * When true, allow a much higher row cap (career compare, multi-team seasons).
   * Only internal callers (e.g. compare_players_career) should set this.
   */
...
export async function getFgSeasonLines(pool: pg.Pool, input: FgSeasonLineInput): Promise<Record<string, unknown>[]> {
