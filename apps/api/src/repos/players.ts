...
import {
  narrowCandidatesByGenerationalHint,
  normalizePlayerNameQuery,
  stripGenerationalSuffixFromQuery,
} from '../lib/playerNameQuery.js';
 ...
 */
function foldLowerSql(expr: string): string {
...
export async function resolvePlayer(pool: pg.Pool, input: ResolvePlayerInput): Promise<Record<string, unknown>> {
  ...
  if (parts.length >= 2) {
    ...
    rows = rowsToJson(r1 as Record<string, unknown>[]);
  }
  ...
  if (rows.length === 0) {
    ...
    rows = rowsToJson(r2 as Record<string, unknown>[]);
  }
...
export type FgCareerDisambiguationHint = {
  ...
  /** Sum of FanGraphs season WAR (batting + pitching); used to rank ambiguous name matches. */
  career_fwar: number;
};
...
export async function resolvePlayerIdFromQuery(
  pool: pg.Pool,
  name_query: string
): Promise<{ player_id: number } | { error: string }> {
  const r = await resolvePlayer(pool, { name_query, limit: NAME_QUERY_CANDIDATE_LIMIT });
  let candidates = r.candidates as Record<string, unknown>[] | undefined;
