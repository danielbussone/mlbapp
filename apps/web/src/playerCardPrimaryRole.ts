...
/**
 * Guess batting vs pitching tab from FanGraphs **career** rows (cheap `last_seasons=1` fetches still include career).
 * Heuristic only — URL / user toggle always wins when set explicitly.
 */
export function inferPrimaryCardRole(bat: FgBattingCardApi, pit: FgBattingCardApi): 'batting' | 'pitching' {
  const bc = bat.career;
  const pc = pit.career;
