...
import styles from './BatPathSummary.module.css';

/** At or below this **container** width, bat-path tiles stack in one column (was viewport `@media 600px`). */
export const BAT_PATH_GRID_STACK_BREAKPOINT_PX = 400;

/** Swing tilt tile body style (`BatPathSummary`). */
const SWING_TILT_FIGURE: SwingTiltFigureVariant = 'silhouette';
...
export function BatPathSummary({ batPath }: { batPath: BatPathApiRow }) {
  ...
  /** When true, single column (4 stacked tiles); when false, 2×2 grid. */
  const [stackPathTiles, setStackPathTiles] = useState(false);
  ...
  useLayoutEffect(() => {
    ...
    const sync = () => {
      ...
      setStackPathTiles(w > 0 && w <= BAT_PATH_GRID_STACK_BREAKPOINT_PX);
    };
  ...
  return (
    <Box ref={containerRef} sx={{ width: '100%', minWidth: 0 }}>
      ...
      <Typography variant="caption" color="text.secondary" className={styles.lead}>
      ...
      <Box
        ...
        sx={{
          gridTemplateColumns: stackPathTiles ? '1fr' : 'repeat(2, minmax(0, 1fr))',
        }}
      >
        <BatSpeedGauge
        ...
        <SwingTiltGraphic
          ...
          batterStand={batPath.batter_stand}
          figureVariant={SWING_TILT_FIGURE}
        />
      </Box>
    </Box>
  );
