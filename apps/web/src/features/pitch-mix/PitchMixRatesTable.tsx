...
import styles from './PitchMixRatesTable.module.css';
...
export function PitchMixRatesTable({ rows }: { rows: Record<string, unknown>[] }) {
  ...
  return (
    <TableContainer className={fgTableShell.fgTableWrap}>
      <Table size="small" className={fgTableShell.fgTableDense}>
        <TableHead>
          <TableRow>
            ...
            <TableCell align="right">
              <Tooltip title="Swings ÷ all pitches of this type (fouls count as swings)." arrow placement="top">
                <Box component="span" className={styles.helpSpan}>
                  Swing%
                </Box>
              </Tooltip>
            </TableCell>
        ...
        <TableBody>
          {rows.map((row, i) => {
            ...
            return (
              <TableRow key={i}>
                <TableCell>
                  <Box className={styles.pitchCell}>
                    <Box
                      className={styles.pitchSwatch}
                      style={{
                        width: 16,
                        height: 7,
                        backgroundColor: pitchTypeMovementColor(pt),
                      }}
                    />
                    {pitchTypeName(pt)}
                  </Box>
