    ...
    <Box sx={{ py: 0.5 }}>
      ...
      {showSlider && (
        ...
        >
          <Box
            sx={{
              width: `calc(100% - ${1.5 * SLIDER_TRACK_INSET_PX}px)`,
              maxWidth: '100%',
            }}
          >
            <Slider
...
export function LeaguePercentilesPanel({ playerId, season, cardRole, positionCode: _positionCode, pitchTypes }: Props) {
  ...
  if (data.role === 'batter') {
    ...
    return (
      <Box sx={{ mb: 1, p: 1.25, border: 1, borderColor: 'divider', borderRadius: 1, bgcolor: 'background.paper' }}>
            ...
            {types.map((pt) => {
              ...
              return (
                <Box key={pt} sx={{ mb: 0.5 }}>
                  <Stack direction="row" alignItems="center" spacing={0.25} sx={{ minHeight: 28 }}>
                    <IconButton
                      size="small"
        ...
        {totalGroup != null ? (
          <>
            ...
            {perPositionGroups.map((g) => {
              ...
              return (
                <Box key={g.position_key} sx={{ mb: 0.5 }}>
                  <Stack direction="row" alignItems="center" spacing={0.25} sx={{ minHeight: 28 }}>
                    <IconButton
                      size="small"
                      aria-expanded={expanded}
                  ...
                  <Collapse in={expanded} timeout="auto" unmountOnExit>
                    <Box sx={{ pl: 3.5 }}>{renderOrderedFieldingRows(g.percentiles)}</Box>
                  </Collapse>
                </Box>
              );
            })}
